...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`http`](https://crates.io/crates/http).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `http` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

HTTP types (`Request`, `Response`, `Uri`, `HeaderMap`)
...
Original work copyrighted by the upstream `http` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
