# Claude Code Project Instructions — http-kotlin

## Project Overview

This is **http-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`http`](https://crates.io/crates/http).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/http/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/http>
